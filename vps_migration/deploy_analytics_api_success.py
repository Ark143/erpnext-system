...
op.open(req)
print("Deployed Server Script 'VM Get Analytics Dashboard' successfully!")

res = op.open(urllib.request.Request(f"{URL}/api/method/vm_get_analytics_dashboard", headers=H))
out = json.loads(res.read().decode())
print("\n>>> LIVE SERVER SCRIPT API OUTPUT <<<")
print("Status:", out.get('message', {}).get('status'))
print("KPIs:", json.dumps(out.get('message', {}).get('kpis'), indent=2))
print("Company sales count:", len(out.get('message', {}).get('company_sales', [])))
print("Top items count:", len(out.get('message', {}).get('top_items', [])))
print("Recent ops count:", len(out.get('message', {}).get('recent_ops', [])))
