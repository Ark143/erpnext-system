...
def vm_pos_history():
    ...
    rows = frappe.get_all(
        ...
        fields=['name', 'posting_date', 'customer_name', 'custom_customer_vehicle as vehicle',
                ...
                'company', 'creation', 'status', 'remarks', 'owner as cashier', 'docstatus', 'consolidated_invoice'],
    ...
    states = recon_states(rows)
    for r in rows:
        r.update(states[r['name']])
        c = r.get('creation') or ''
        r['timestamp'] = str(c)[:19] if c else str(r.get('posting_date') or '')
        r['pos_invoice'] = r['name']
        mop = frappe.db.get_value('Sales Invoice Payment', {'parent': r['name'], 'parenttype': 'POS Invoice'}, 'mode_of_payment')
