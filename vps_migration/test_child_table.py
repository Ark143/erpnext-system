...
script_code = """
items = frappe.get_all('Sales Invoice Item', filters={'parenttype': 'Sales Invoice'}, fields=['item_name', 'qty', 'amount'], limit_page_length=50)
frappe.response['message'] = {'count': len(items), 'sample': items[0] if items else None}
"""
...
print("Child table test:", res.read().decode())
