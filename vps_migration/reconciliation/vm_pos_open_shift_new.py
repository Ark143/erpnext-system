...
def vm_pos_open_shift():
    ...
    # Reuse the cashier's open shift; never abandon it or close another user.
    existing = recon_opening(user)
    if existing:
        if existing.company != company or existing.pos_profile != profile_name:
            frappe.throw('Close your existing shift ' + existing.name + ' before opening a different POS profile.')
        if recon_pending_closing(existing):
            frappe.throw('A closing entry is already processing for this shift.')
        summary = recon_summary(existing, recon_candidates(existing)['invoices'])
        frappe.response['message'] = {'name': existing.name, 'pos_profile': existing.pos_profile,
            'company': existing.company, 'status': existing.status,
            'opening_amount': summary['opening_amount'], 'mode_of_payment': mop}
        return
