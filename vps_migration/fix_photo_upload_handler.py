...
# 1. Create Server Script API: vm_save_receiver_photo
photo_api_script = '''
def save_photo():
...
save_doc("Server Script", "VM Save Receiver Photo", {
    "script_type": "API",
    "api_method": "vm_save_receiver_photo",
    "allow_guest": 1,
    "disabled": 0,
    "script": photo_api_script
})

# 2. Update Client Script to use vm_save_receiver_photo
client_script_code = """
...
function vm_open_receiver_dialog(frm) {
    ...
    let d = new frappe.ui.Dialog({
        ...
        primary_action: function() {
            ...
            if (photoDataUrl) {
                frappe.call({
                    method: 'vm_save_receiver_photo',
                    args: {
                        docname: frm.doc.name,
                        filename: 'receiver_proof_' + (frm.doc.name || 'se').replace(/[^a-zA-Z0-9_-]/g, '') + '_' + Date.now() + '.jpg',
                        filedata: photoDataUrl
                    },
                    callback: function(r) {
                        let res = r.message;
                        if (res && res.ok && res.file_url) {
                            frm.set_value('custom_receiver_photo', res.file_url);
                        }
                        d.hide();
                        frm.save();
...
print("Successfully deployed vm_save_receiver_photo API and updated Client Script!")
