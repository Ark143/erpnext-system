...
def vm_get_analytics_dashboard():
    ...
    # Format sorted lists
    company_sales_list = []
    for k, v in sorted(sales_by_co.items(), key=lambda x: x[1], reverse=True)[:6]:
        company_sales_list.append({'company': k, 'total': v})
        
    monthly_sales_list = []
    for k in sorted(monthly_sales_agg.keys()):
        monthly_sales_list.append({'month': k, 'total': monthly_sales_agg[k]})
        
    top_supp_list = []
    for k, v in sorted(supp_by_co.items(), key=lambda x: x[1], reverse=True)[:6]:
        top_supp_list.append({'supplier': k, 'amount': v})
        
    wh_stock_list = []
    for k, v in sorted(wh_stock.items(), key=lambda x: x[1], reverse=True)[:6]:
        wh_stock_list.append({'warehouse': k, 'value': v})
        
    top_items_list = []
    for k, v in sorted(items_agg.items(), key=lambda x: x[1]['amount'], reverse=True)[:6]:
        top_items_list.append({'name': k, 'qty': v['qty'], 'amount': v['amount']})
        
    top_makes_list = []
    for k, v in sorted(veh_makes_agg.items(), key=lambda x: x[1], reverse=True)[:6]:
        top_makes_list.append({'make': k, 'count': v})
