"""
Executive Dashboard API Endpoint for ERPNext
Whitelisted method for /api/method/executive_dashboard
"""
import frappe
...
def executive_dashboard(view=None, company=None, months=12, fy=None):
    view = view or frappe.form_dict.get('view') or 'meta'
    company = company or frappe.form_dict.get('company') or 'Ultra MRF Dau Main'
    months = cint(months or frappe.form_dict.get('months')) or 12
    fy = fy or frappe.form_dict.get('fy') or ''

    if view == 'meta':
        return get_meta(company)
    elif view == 'exec_summary':
        return get_exec_summary(company, fy)
    elif view == 'sales':
        return get_sales(company, months)
    elif view == 'procurement':
        return get_procurement(company, months)
    elif view == 'finance':
        return get_finance(company, months)
    elif view == 'budget':
        return get_budget(company, months)
    elif view == 'approvals':
        return get_approvals(company)
    elif view == 'operations':
        return get_operations(company, months)
    elif view == 'alerts':
        return get_alerts(company)
    else:
        return {"error": f"Unknown view: {view}"}

def get_meta(company):
...
def get_exec_summary(company, fy):
    fy_name = fy or "2026"
...
def get_procurement(company, months):
    ytd_inv = frappe.db.sql("""
        SELECT COALESCE(SUM(grand_total), 0) as tot
        FROM "tabPurchase Invoice"
        WHERE company = %s AND docstatus = 1 AND posting_date >= DATE_TRUNC('year', CURRENT_DATE)
...
def get_finance(company, months):
    labels = ["2026-08"]
...
def get_approvals(company):
    cards = []
    doctypes = [
        ("Vehicle Job Order", "grand_total", "creation"),
    ...
    for dt, amt_col, date_col in doctypes:
        ...
        try:
            ...
            amt_expr = amt_col if frappe.db.has_column(dt, amt_col) else "0"
            date_expr = date_col if frappe.db.has_column(dt, date_col) else "creation"
            
            if has_company:
                sql = f"""
                    SELECT 
                        COUNT(name) as cnt, 
                        COALESCE(SUM({amt_expr}), 0) as tot, 
                        COALESCE(MAX({amt_expr}), 0) as high,
                        MIN({date_expr}) as oldest,
                        AVG(GREATEST(0, EXTRACT(DAY FROM (NOW() - {date_expr}::timestamp)))) as avg_wait
                    FROM "tab{dt}"
                    WHERE company = %s AND docstatus = 0
                """
                drafts = frappe.db.sql(sql, (company,), as_dict=True)[0]
            else:
                sql = f"""
                    SELECT 
                        COUNT(name) as cnt, 
                    ...
                    WHERE docstatus = 0
                """
                drafts = frappe.db.sql(sql, as_dict=True)[0]
            
            oldest_days = 0
            if drafts.get('oldest'):
                try:
                    d_val = getdate(drafts['oldest'])
                    oldest_days = max(0, (getdate(nowdate()) - d_val).days)
                except Exception:
                    oldest_days = 0
...
def get_operations(company, months):
    top_cust = frappe.db.sql("""
        SELECT customer_name as name, COALESCE(SUM(grand_total), 0) as amount
        FROM "tabSales Invoice"
        WHERE company = %s AND docstatus = 1
