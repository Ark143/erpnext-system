...
function getCsrfToken() {
  if (typeof frappe !== "undefined") {
    if (frappe.csrf_token && frappe.csrf_token !== "None") return frappe.csrf_token;
    if (frappe.boot && frappe.boot.csrf_token && frappe.boot.csrf_token !== "None") return frappe.boot.csrf_token;
  }
  if (typeof csrf_token !== "undefined" && csrf_token && csrf_token !== "None") return csrf_token;
  if (window.csrf_token && window.csrf_token !== "None") return window.csrf_token;
  const match = document.cookie.match(/(?:^|;\s*)csrf_token=([^;]+)/);
  if (match && match[1] && match[1] !== "None") return decodeURIComponent(match[1]);
  return "";
}
...
async function api(method, params, httpMethod) {
  ...
  if (csrf) {
    fetchOptions.headers["X-Frappe-CSRF-Token"] = csrf;
  }
  
  if (isPost) {
    fetchOptions.method = "POST";
    fetchOptions.headers["Content-Type"] = "application/json";
    if (csrf && params && typeof params === "object" && !Array.isArray(params)) {
...
var POS = {
  ...
  async showCloseShiftModal() {
    ...
    overlay.innerHTML = '<div class="vpos-closing-modal">Loading unreconciled invoices...</div>';
    document.body.appendChild(overlay);
    try {
      const result = await api("vm_pos_get_shift", {company:this.company || ""});
      if (!result) throw new Error(api.lastError || "Unable to load the shift. Please try again.");
      if (!result.has_open_shift) throw new Error("No active shift exists for your account. Open your shift before reconciling invoices.");
      const shift = result.shift;
      this.openingEntry = shift.name;
      this.openingAmount = Number(shift.opening_amount || 0);
      const invoices = shift.invoices || [];
      const selected = new Set(invoices.map(i => i.name));
      const payments = shift.payments || [];
      const pending = shift.pending_closing;
      const others = shift.other_profile_invoices || [];
      overlay.innerHTML = `<div class="vpos-closing-modal vpos-recon-modal">
        <div class="vpos-closing-title">Close Shift & Daily Reconciliation</div>
        <div class="vpos-closing-sub">${esc(shift.user)} · ${esc(shift.company)}<br>${esc(shift.name)} · ${esc(shift.pos_profile)}<br>Shift opened: ${esc(shift.period_start_date)}</div>
        ${pending ? `<div class="vpos-recon-warning">Closing entry <a href="/desk/pos-closing-entry/${encodeURIComponent(pending.name)}" target="_blank" rel="noopener">${esc(pending.name)}</a> is ${esc(pending.status)}. Complete, retry, or cancel that entry before closing again.</div>` : ''}
        <div class="vpos-recon-note">All eligible unreconciled invoices for your account and POS profile are selected automatically. Older unreconciled invoices are marked below.</div>
        <div class="vpos-recon-actions"><button type="button" id="recon-all">Select All</button><button type="button" id="recon-none">Deselect All</button><button type="button" id="recon-refresh">Refresh Invoices</button><span id="recon-selected-count"></span></div>
        <div class="vpos-recon-invoices">${invoices.length ? invoices.map((i,n) => `<label class="vpos-recon-invoice"><input type="checkbox" data-invoice-index="${n}" checked ${pending ? 'disabled' : ''}><span><b>${esc(i.name)}</b><small>${esc(i.posting_date)} · ${esc(i.customer_name || i.customer)}${i.is_return ? ' · Return' : ''}${i.earlier_invoice ? ' · Earlier / unreconciled' : ''}</small></span><strong>${money(i.grand_total)}</strong></label>`).join('') : '<p>No eligible unreconciled invoices for this POS profile.</p>'}</div>
        ${others.length ? `<details class="vpos-recon-warning"><summary>${others.length} unreconciled invoice(s) use a different POS profile</summary><p>ERPNext requires a separate closing entry for each POS profile. These invoices are not included in this shift.</p>${others.map(i=>`<div>${esc(i.name)} · ${esc(i.pos_profile)} · ${money(i.grand_total)}</div>`).join('')}</details>` : ''}
        <div class="vpos-summary-row"><span>Selected Invoice Sales</span><b id="recon-sales"></b></div>
        <div class="vpos-summary-row"><span>Collected, After Change / Refunds</span><b id="recon-collected"></b></div>
        <div class="vpos-summary-row"><span>Expected Cash in Drawer</span><b id="recon-cash"></b></div>
        <div class="vpos-recon-note">Count physical cash and verify card / e-wallet totals. Electronic payments are not included in the cash drawer total.</div>
        <div class="vpos-recon-payments"><table><thead><tr><th>Payment Method</th><th>Opening</th><th>Expected</th><th>Actual / Verified</th><th>Difference</th></tr></thead><tbody>${payments.map((p,n)=>`<tr><td>${esc(p.mode_of_payment)}${p.is_cash ? ' (cash)' : ''}</td><td>${money(p.opening_amount)}</td><td data-expected="${n}"></td><td><input class="vpos-recon-count" data-payment-index="${n}" aria-label="${esc(p.mode_of_payment)} actual amount" type="number" step="0.01" ${p.is_cash ? 'min="0"' : ''} placeholder="Enter amount" value="${p.is_cash ? '' : Number(p.expected_amount).toFixed(2)}" ${pending ? 'disabled' : ''}></td><td data-difference="${n}">—</td></tr>`).join('')}</tbody></table></div>
        <div id="recon-summary-note" class="vpos-recon-note"></div>
        <div style="display:flex;gap:10px;margin-top:18px"><button class="vpos-shift-btn" id="shift-close-confirm" ${pending ? 'disabled' : ''}>Submit & Close Shift</button><button class="vpos-shift-btn" id="shift-close-cancel" style="background:#30363d">Cancel</button></div>
        <div class="vpos-shift-err" id="closing-err" role="alert"></div>
      </div>`;
      const err = overlay.querySelector('#closing-err');
      const submit = overlay.querySelector('#shift-close-confirm');
      const update = () => {
        const chosen = invoices.filter(i=>selected.has(i.name));
        const amounts = Object.fromEntries(payments.map(p=>[p.mode_of_payment,Number(p.opening_amount || 0)]));
        let sales=0, collected=0;
        chosen.forEach(i=>{sales+=Number(i.grand_total||0);collected+=Number(i.collected_amount||0);Object.entries(i.payment_amounts || {}).forEach(([m,a])=>{amounts[m]=(amounts[m]||0)+Number(a||0);});});
        overlay.querySelector('#recon-selected-count').textContent = `${chosen.length} of ${invoices.length} selected`;
        overlay.querySelector('#recon-sales').textContent=money(sales);
        overlay.querySelector('#recon-collected').textContent=money(collected);
        overlay.querySelector('#recon-cash').textContent=money(payments.filter(p=>p.is_cash).reduce((sum,p)=>sum+(amounts[p.mode_of_payment]||0),0));
        payments.forEach((p,n)=>{
          const expected=amounts[p.mode_of_payment]||0;
          const input=overlay.querySelector(`[data-payment-index="${n}"]`);
          if(!p.is_cash && !input.dataset.edited) input.value=expected.toFixed(2);
          overlay.querySelector(`[data-expected="${n}"]`).textContent=money(expected);
          const diff=overlay.querySelector(`[data-difference="${n}"]`);
          if(input.value===''){diff.textContent='—';}else{const value=Number(input.value)-expected;diff.textContent=money(value);diff.style.color=Math.abs(value)<0.005?'#16c784':'#ffb454';}
        });
        overlay.querySelector('#recon-summary-note').textContent=chosen.length===invoices.length ? 'Automatic selection: all eligible invoices included.' : `${invoices.length-chosen.length} invoice(s) will remain unreconciled for a later closing entry.`;
        submit.disabled=!!pending || (!!invoices.length && !chosen.length);
      };
      overlay.querySelectorAll('[data-invoice-index]').forEach(input=>{input.onchange=()=>{const name=invoices[Number(input.dataset.invoiceIndex)].name;input.checked?selected.add(name):selected.delete(name);update();};});
      overlay.querySelectorAll('[data-payment-index]').forEach(input=>{input.oninput=()=>{input.dataset.edited='true';update();};});
      const selectAll = checked => {if(pending)return;overlay.querySelectorAll('[data-invoice-index]').forEach(input=>{input.checked=checked;});selected.clear();if(checked)invoices.forEach(i=>selected.add(i.name));update();};
      overlay.querySelector('#recon-all').onclick=()=>selectAll(true);
      overlay.querySelector('#recon-none').onclick=()=>selectAll(false);
      overlay.querySelector('#recon-refresh').onclick=()=>{overlay.remove();self.showCloseShiftModal();};
      ...
      submit.onclick=async()=>{
        ...
        for(let n=0;n<payments.length;n++){
          const input=overlay.querySelector(`[data-payment-index="${n}"]`);
          if(input.value==='' || !Number.isFinite(Number(input.value)) || !input.checkValidity()){err.textContent='Enter a valid actual amount for '+payments[n].mode_of_payment+'.';input.focus();return;}
          counted[payments[n].mode_of_payment]=Number(input.value);
        }
        submit.disabled=true;submit.textContent='Submitting closing entry...';
        try{
          const names=invoices.filter(i=>selected.has(i.name)).map(i=>i.name);
          const result=await api('vm_pos_close_shift',{data:JSON.stringify({opening_entry:shift.name,invoice_names:names,invoice_versions:Object.fromEntries(invoices.filter(i=>selected.has(i.name)).map(i=>[i.name,i.modified])),selection_mode:names.length===invoices.length?'all':'manual',counted_amounts:counted})});
          if(!result || !result.name)throw new Error(api.lastError||'Unable to close the shift. Refresh the summary and try again.');
          overlay.remove();
          alert(`Closing Entry: ${result.name}\nStatus: ${result.status}\nInvoices: ${result.total_invoices}\nSales: ${money(result.grand_total)}\nCash Counted: ${money(result.closing_amount)}\nCash Difference: ${money(result.difference)}\nInvoices left unreconciled: ${result.remaining_invoices||0}`);
          if(result.status==='Submitted'){self.openingEntry=null;self.openingAmount=0;self.logout();}
          else {await self.showCloseShiftModal();}
        }catch(e){err.textContent=e.message||String(e);submit.disabled=false;submit.textContent='Submit & Close Shift';}
      };
      update();
    } catch(e) {
      overlay.innerHTML='<div class="vpos-closing-modal"><div class="vpos-shift-err" role="alert"></div><button class="vpos-shift-btn">Close</button></div>';
      overlay.querySelector('[role="alert"]').textContent=e.message||String(e);
  ...
  build() {
      ...
      <aside class="vpos-ticket" id="vpos-ticket-panel">
        ...
        <div class="vpos-tender">
          ...
          <div class="vpos-quick">
            <button type="button" class="vpos-q-exact" data-q="exact">Exact</button>
            <button type="button" data-q="100">+100</button>
            <button type="button" data-q="200">+200</button>
            <button type="button" data-q="500">+500</button>
            <button type="button" data-q="1000">+1000</button>
          </div>
    ...
    r.querySelectorAll(".vpos-pay-btn").forEach(btn => {
      btn.onclick = () => {
        ...
        if (pInput && (self.payment_method !== "Cash" || flt(pInput.value) <= 0 || flt(pInput.value) < tot)) {
          pInput.value = tot;
          self.totals();
        }
      };
    ...
    r.querySelectorAll(".vpos-quick button").forEach(b => b.onclick = () => self.quickAdd(b.getAttribute("data-q")));
  ...
  renderHistoryList() {
    ...
    listEl.innerHTML = list.map(t => {
      ...
      const escapeRecon = v => String(v ?? "").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
      const reconLabel = t.reconciliation_status || 'Status unavailable';
      const reconBadge = `<span class="vpos-hist-tag ${t.is_reconciled ? 'paid' : 'vpos-recon-pending'}">${escapeRecon(reconLabel)}</span>`;
      const closingLink = t.closing_entry ? `<a class="vpos-hist-tag" href="/desk/pos-closing-entry/${encodeURIComponent(t.closing_entry)}" target="_blank" rel="noopener">${escapeRecon(t.closing_entry)}</a>` : '';

      const posLink = t.pos_invoice ? `<span class="vpos-hist-tag desk-link" onclick="window.open('/desk/pos-invoice/${encodeURIComponent(t.pos_invoice)}', '_blank')">🔗 ${t.pos_invoice}</span>` : "";
      ...
      <div class="vpos-hist-card">
        <div class="vpos-hist-top">
          <div class="vpos-hist-code" onclick="window.open('/desk/pos-invoice/${encodeURIComponent(t.name)}', '_blank')" title="Open POS Invoice in Desk">${t.name}</div>
  ...
  card(it, box) {
    ...
    let thumbHtml = "";
    if (it.image && it.image !== 'undefined' && it.image !== 'null' && String(it.image).trim() !== '') {
  ...
  add(code, name, rate, uom) {
    ...
    // Auto-update paid input to match total if not custom cash
    const pInput = document.querySelector(".vpos-paid");
    if (pInput) {
      const curPaid = flt(pInput.value);
      if (curPaid <= 0 || curPaid === prevTot || this.payment_method !== "Cash") {
        let newTot = 0;
        this.cart.forEach(c => newTot += flt(c.qty) * flt(c.rate) - flt(c.discount_amount));
        pInput.value = newTot;
      }
    }
  ...
  totals() {
    ...
    let paid = flt(paidInp ? paidInp.value : 0);
    
    // Auto-fill total if cart has items and paid is 0
    if (this.cart.length && paid <= 0) {
      paid = tot;
      if (paidInp) paidInp.value = tot;
    }
    ...
    if (chargeBtn) {
      chargeBtn.disabled = !this.cart.length;
    }
  ...
  quickAdd(q) {
    ...
    if (q === "exact") {
      document.querySelector(".vpos-paid").value = tot;
    } else {
  ...
  async charge() {
    const tot = this.total || 0;
    if (!this.cart.length) { alert("Cart is empty. Please select items from catalog first."); return; }

    const pInp = document.querySelector(".vpos-paid");
    let paid = flt(pInp ? pInp.value : 0) || 0;

    if (paid <= 0) {
      paid = tot;
      if (pInp) pInp.value = paid;
      this.totals();
    }
    ...
    if (chargeBtn) {
      ...
      chargeBtn.innerHTML = "⏳ Processing Payment...";
    }

    try {
      await this.submit(tot, paid);
    } finally {
      if (chargeBtn) {
        chargeBtn.disabled = !this.cart.length;
        chargeBtn.innerHTML = "&#9889; Charge &amp; Pay";
      }
    }
  },
  ...
  async submit(tot, paid) {
    ...
    const payload = {
      opening_entry: this.openingEntry, pos_profile: this.posProfile,
  ...
  _buildReceiptDom(sale) {
    ...
    div.innerHTML = this._buildPrintMarkup(sale);
    return div;
  },
  ...
  showReceiptModal(sale) {
    ...
    overlay.innerHTML = `
      <div class="vpos-receipt-dialog">
        ...
        <div class="vpos-receipt-dialog-foot">
          ...
          ${sale.invoice_no && sale.invoice_no !== 'OPEN-CART' ? `<button class="vpos-receipt-btn-desk" onclick="window.open('/desk/pos-invoice/${encodeURIComponent(sale.invoice_no)}', '_blank')">🔗 Desk</button>` : ''}
    ...
    if (winBtn) {
      winBtn.onclick = () => {
        self.openPrintWindow(self._buildPrintMarkup(sale));
      };
    }
  },

  // Receipt layout v2: one template, isolated print document, measured roll height.
  _buildPrintMarkup(sale) {
    const esc = value => String(value ?? "").replace(/[&<>"']/g, c => ({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
    const peso = v => "₱ " + parseFloat(v || 0).toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    ...
    if (itemList && itemList.length) {
      itemList.forEach(it => {
        const amt = it.amount != null ? flt(it.amount) : (flt(it.qty) * flt(it.rate) - flt(it.discount_amount || 0));
        itemsRows += `
          <tr style="border-bottom:1px dashed #e2e8f0;">
            <td style="padding:4px 0;vertical-align:top;font-weight:700;width:62%;">${esc(it.item_name || it.item_code)}<br>
              <span style="font-size:11px;font-weight:400;color:#555;">${esc(it.qty)} ${esc(it.uom || 'PC')} × ${peso(it.rate)}</span>
            </td>
            <td style="padding:4px 0;vertical-align:top;text-align:right;font-weight:700;width:38%;white-space:nowrap;">${peso(amt)}</td>
          </tr>`;
      });
    } else if (sale.items_html) {
      const legacy = document.createElement("div");
      legacy.innerHTML = sale.items_html;
      itemsRows = '<tr><td colspan="2">' + esc(legacy.textContent) + '</td></tr>';
    } else {
      itemsRows = '<tr><td colspan="2" style="color:#555;font-style:italic;">No items.</td></tr>';
    }

    const receiptMarkup = `
      <div class="vpos-receipt-print-wrapper" style="width:100%;max-width:80mm;margin:0 auto;box-sizing:border-box;overflow-wrap:anywhere;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Courier New',monospace;font-size:12px;line-height:1.35;color:#000;background:#fff;padding:3mm 4mm;">
        <div style="font-size:16px;font-weight:900;text-align:center;text-transform:uppercase;color:#000;">${esc(company)}</div>
        ...
        <div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Bill No:</span><b>${esc(invoiceNo)}</b></div>
        <div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Date &amp; Time:</span><span>${esc(date)} ${esc(time)}</span></div>
        <div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Cashier:</span><span>${esc(cashier)}</span></div>

        <div style="border-top:1px dashed #777;margin:5px 0;"></div>
        <div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Customer:</span><b>${esc(customer)}</b></div>
        ${vehicle ? `<div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Vehicle:</span><b>${esc(vehicle)}</b></div>` : ''}
        ...
        <table style="width:100%;table-layout:fixed;border-collapse:collapse;margin:4px 0;">
          <thead>
            <tr>
              <th style="width:62%;border-bottom:1px dashed #000;padding:3px 0;font-size:10.5px;text-align:left;font-weight:700;">DESCRIPTION</th>
        ...
        ${discountAmount ? `<div style="border-top:1px dashed #777;margin:4px 0;"></div><div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Discount:</span><span>-${discountAmount}</span></div>` : ''}

        <div style="border-top:1.5px solid #000;margin:6px 0;"></div>
        <div style="display:flex;gap:3mm;justify-content:space-between;align-items:center;font-size:16px;font-weight:900;color:#000;padding:3px 0;">
          <span>TOTAL:</span>
          <span>${totalAmount}</span>
        </div>
        <div style="border-top:1px dashed #777;margin:5px 0;"></div>

        <div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Payment Method:</span><b>${esc(paymentMethod)}</b></div>
        ${paidAmount ? `<div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Amount Tendered:</span><span>${paidAmount}</span></div>` : ''}
        ${changeAmount ? `<div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Change Due:</span><b style="font-size:13px;">${changeAmount}</b></div>` : ''}

        ${remarks ? `<div style="border-top:1px dashed #777;margin:5px 0;"></div><div style="display:flex;gap:3mm;justify-content:space-between;margin:2px 0;font-size:11.5px;"><span>Remarks:</span><span>${esc(remarks)}</span></div>` : ''}
        ...
        <div style="text-align:center;font-size:10px;color:#444;margin-top:6px;line-height:1.3;">
          Thank you for choosing ${esc(company)}!<br>
  ...
  _receiptPrintCss() {
    return `
      * { box-sizing: border-box; }
      ...
      body { width:80mm; max-width:100%; }
      .vpos-receipt-print-wrapper { width:80mm; max-width:100%; }
      .vpos-receipt-print-wrapper > div > :last-child:not(:first-child) {
        min-width:0; text-align:right; overflow-wrap:anywhere;
      }
      .vpos-receipt-print-wrapper > div > :first-child:not(:last-child) { flex-shrink:0; }
      table { table-layout:fixed; }
      tr { break-inside:avoid; }
      @media print {
        html, body { height:auto; min-height:0; overflow:visible; }
        .vpos-receipt-print-wrapper { margin:0 !important; break-inside:auto; }
      }
    `;
  },

  triggerPrint(sale) {
    sale = sale || this._currentModalSale || this._recentSale || (this.cart.length ? this._buildOpenCartSale() : null);
    if (!sale) { alert("No transaction to print."); return; }
    const markup = this._buildPrintMarkup(sale);
    // Print only this frame, never the POS page or its enclosing Desk iframe.
    let frame = document.getElementById("vpos-receipt-print-frame");
    if (!frame) {
      frame = document.createElement("iframe");
      frame.id = "vpos-receipt-print-frame";
      frame.title = "Receipt print document";
      frame.style.cssText = "position:fixed;left:-10000px;top:0;width:80mm;height:1px;border:0;";
      document.body.appendChild(frame);
    }
    this._writeReceiptDocument(frame.contentWindow, markup);
  },

  _writeReceiptDocument(target, markup) {
    const doc = target.document;
    doc.open();
    doc.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>POS Receipt</title><style>${this._receiptPrintCss()}</style><style id="receipt-page-size"></style></head><body>${markup}</body></html>`);
    doc.close();
    const ready = doc.fonts ? doc.fonts.ready : Promise.resolve();
    ready.then(() => target.requestAnimationFrame(() => {
      const receipt = doc.querySelector(".vpos-receipt-print-wrapper");
      // CSS pixels are 96/inch. Extra 2mm prevents rounding from creating page two.
      const heightMm = Math.max(40, Math.ceil(receipt.getBoundingClientRect().height * 25.4 / 96) + 2);
      doc.getElementById("receipt-page-size").textContent = `@page { size:80mm ${heightMm}mm; margin:0; }`;
      target.requestAnimationFrame(() => { target.focus(); target.print(); });
    }));
  },

  openPrintWindow(receiptMarkup) {
    const w = window.open("", "_blank", "width=420,height=680,menubar=no,toolbar=no,location=no,status=no");
    if (w) this._writeReceiptDocument(w, receiptMarkup);
    else alert("Popup blocked. Please use Print Receipt or allow popups.");
  },
