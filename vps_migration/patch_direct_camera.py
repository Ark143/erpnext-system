...
new_login_block = """buildLogin(){
    ...
    r.innerHTML=`<div class="vpos-login" id="vpos-login">
      <div class="vpos-login-card">
        ...
        <label class="vpos-li-qr" id="vpos-li-scan-lbl" for="vpos-li-file" style="display:flex;align-items:center;justify-content:center;cursor:pointer;margin-bottom:0;">&#128247; Scan QR Code / Camera</label>
        <input type="file" accept="image/*" capture="environment" id="vpos-li-file" style="position:absolute;left:-9999px;width:1px;height:1px;opacity:0;">
    ...
    const scanLbl = r.querySelector("#vpos-li-scan-lbl");
    const err = r.querySelector("#vpos-li-err");

    r.querySelector("#vpos-li-go").onclick=()=>self.doLogin(userInput.value, passInput.value);
    
    // Scan button behavior: if live video stream API is supported, intercept and open live stream; otherwise let native label click open camera capture
    scanLbl.onclick = (e) => {
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        e.preventDefault();
        self.openScanner();
      } else {
        err.textContent = "Opening camera capture...";
      }
    };
  ...
  async decodeImage(file){
    const err=document.getElementById("vpos-li-err");
    if(!file)return;
    err.textContent="Decoding QR code...";
    try {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          err.textContent = "No QR found in image. Please try again or paste badge code.";
        };
  ...
  openScanner(){
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } }).then(stream => {
      ...
      if (sBox) sBox.style.display = "none";
      err.textContent = "Camera access denied. Please click above or paste badge code.";
    });
  },
...
with open("c:/Users/josem/erpnext-system/vps_migration/temp_patched4.html", "w", encoding="utf-8") as f:
    f.write(patched_html)

node_cmd = ["node", "-e",
    "const fs=require('fs');const h=fs.readFileSync('c:/Users/josem/erpnext-system/vps_migration/temp_patched4.html','utf8');"
...
print("SUCCESSFULLY APPLIED DIRECT CAMERA CAPTURE BINDING")
