...
print("=== Sidebar Link items with missing/empty link_type (these crash generate_route) ===")
for key, sb in wsi.items():
    items = sb.get("items") or []
    for i in items:
        if i.get("type") == "Link" and (i.get("link_type") in (None, "")):
            print(f"  [{key}] label={i.get('label')!r} link_type={i.get('link_type')!r} link_to={i.get('link_to')!r}")
print("scan done")
