...
# point cross-links at a valid existing doctype so creation passes
jo = make_doc("vehicle_job_order")
for f in jo.get("fields",[]):
    if f.get("fieldname")=="estimate": f["options"]="Sales Invoice"
ve = make_doc("vehicle_estimate")
for f in ve.get("fields",[]):
    if f.get("fieldname")=="job_order": f["options"]="Sales Invoice"
