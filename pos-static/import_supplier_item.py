...
csrf = get_csrf()
print("login OK (parallel Supplier/Item)", flush=True)

REMAP = {
    "Supplier": {"supplier_group": {"All Supplier Groups": "Local"}},
...
def import_doc(doctype, export_file, threads=6, max_retries=6):
    ...
    def clean(r):
        ...
        for fld,mp in remap.items():
            if fld in doc and doc[fld] in mp: doc[fld]=mp[doc[fld]]
        return doc
    def do_one(r):
        doc = clean(r)
...
# Supplier + Item in parallel with the main Customer run
PLAN = [
    ("Supplier", "pos-static/export/Supplier.json", 6),
    ("Item", "pos-static/export/Item.json", 6),
]
for dt,f,th in PLAN:
    if import_doc(dt,f,threads=th):
        print(f"STOP on 403 {dt}"); break
print("\nDONE parallel Supplier/Item")
