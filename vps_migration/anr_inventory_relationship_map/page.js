(() => {
  ...
  const url = (dt, name) => '/app/' + dt.toLowerCase().replace(/ /g, '-') + '/' + encodeURIComponent(name);
  ...
  function render() {
    ...
    $('im-results').innerHTML = `<div class="im-item-title"><div><h2>${esc(data.item.item_name)}</h2><div class="im-muted">${link('Item',data.item.name)} · Stock UOM: ${uom}${data.company?' · '+esc(data.company):''}</div></div><span class="im-badge">${data.item.is_stock_item?'Stock item':'Non-stock item'}</span></div>
      ...
      <div class="im-muted">Current stock is the warehouse-level Bin balance; date filters affect movements only. Warehouse cards show the net of displayed movements. Transfers contribute to both inward and outward quantities.</div>
      ...
      <section class="im-panel" role="tabpanel" aria-labelledby="im-tab-2" id="im-panel-2" hidden><p class="im-muted">Current warehouse totals, regardless of movement dates. All quantities use ${uom}.</p>${table(['Warehouse','Company','Actual qty','Reserved qty','Ordered qty','Projected qty'],balances.map(b=>[link('Warehouse',b.warehouse),esc(b.company),num(b.actual_qty),num(b.reserved_qty),num(b.ordered_qty),num(b.projected_qty)]))}</section>
  ...
  function drawGraph() {
    ...
    whs.forEach(w=>{
      ...
      cards+=node(365,whY[w],'im-wh',href,'WAREHOUSE',title,details,'Displayed net: '+num(matching.reduce((n,r)=>n+Number(r.actual_qty),0))+' '+data.item.stock_uom);
    });
