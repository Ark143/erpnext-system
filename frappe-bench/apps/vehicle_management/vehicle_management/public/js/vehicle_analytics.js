frappe.pages["vehicle_analytics"].on_page_load = function (wrapper) {
	...
	page.dashboard_body.html(`
		<div style="padding: 40px; text-align: center; color: var(--text-muted);">
			...
			<div style="margin-top: 10px; font-weight: 600;">Loading Permissions & Analytics...</div>
		</div>
	`);

	// Query user permissions first to dynamically configure company options
	frappe.call({
		method: "vehicle_management.vehicle_management.analytics.get_user_analytics_permissions",
		callback: function (r) {
			...
			if (perm.can_view_all) {
				company_options = ["All Companies", ...(perm.allowed_companies || [])];
			} else {
				if (perm.allowed_companies && perm.allowed_companies.length > 0) {
					company_options = perm.allowed_companies;
				} else {
					company_options = ["No Branch Assigned"];
				}
			}
			...
			page.company_field = page.add_field({
				...
				default: perm.default_company || company_options[0],
			...
			// Access restriction indicator badge
			if (!perm.can_view_all) {
				page.set_indicator(__("Branch Access: {0}", [perm.allowed_companies.join(", ") || "None"]), "blue");
			} else {
				page.set_indicator(__("Consolidated View (All Companies)"), "green");
			}
...
function render_analytics_dashboard(page) {
	const default_comp = page.user_perm && page.user_perm.default_company ? page.user_perm.default_company : "All Companies";
	const company = page.company_field ? page.company_field.get_value() : default_comp;
	...
	frappe.call({
		...
		callback: (r) => {
			if (!r.message) {
				page.dashboard_body.html(`<div class="alert alert-warning" style="margin: 20px;">No data available for the selected filters.</div>`);
				return;
			}
			...
			const perm = data.user_perm || page.user_perm || { can_view_all: true };

			let html = `
			<div class="analytics-dashboard-container" style="padding: 10px 0; font-family: var(--font-stack);">
				<!-- Access Banner for Branch Users -->
				${!perm.can_view_all ? `
				<div class="alert alert-info d-flex align-items-center mb-3" style="border-radius: 8px; font-size: 13px;">
					<i class="fa fa-shield fa-lg mr-2" style="margin-right: 8px; color: #1a56db;"></i>
					<div>
						<strong>Branch-Restricted Access:</strong> Logged in as <strong>${frappe.session.user_fullname || frappe.session.user}</strong>. Showing vehicle analytics scoped exclusively to your assigned branch: <strong>${perm.allowed_companies.join(", ")}</strong>.
					</div>
				</div>
				` : ''}
					...
					<div class="col-md-6 mb-3">
						<div class="card p-3" style="border-radius: 10px; border: 1px solid var(--border-color); background: var(--card-bg);">
							<div style="font-weight: 700; font-size: 14px; margin-bottom: 12px;">
								<i class="fa fa-bar-chart text-primary"></i> ${perm.can_view_all ? 'Total Revenue by Company / Branch' : 'Branch Revenue Performance'}
							</div>
				...
				<div class="row">
					...
					<div class="col-lg-7 mb-3">
						<div class="card" style="border-radius: 10px; border: 1px solid var(--border-color); background: var(--card-bg); overflow: hidden;">
							<div class="card-header bg-light d-flex justify-content-between align-items-center" style="padding: 12px 16px; border-bottom: 1px solid var(--border-color);">
								<div style="font-weight: 700; font-size: 14px; color: var(--text-color);">
									<i class="fa fa-building-o text-info mr-1"></i> ${perm.can_view_all ? 'Branch & Company Performance' : 'Branch Performance Breakdown'}
								</div>
...
function render_top_services_rows(services) {
	...
	services.forEach((s, idx) => {
		const rank_badge = idx === 0 ? "badge-warning" : (idx === 1 ? "badge-secondary" : "badge-light");
		html += `
		<tr>
