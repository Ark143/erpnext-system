...
# Let's find the buildLogin, applyQr, decodeImage, openScanner block in html
...
new_login_block = """buildLogin(){
    ...
    r.querySelector("#vpos-li-file").onchange=e=>{ if(e.target.files && e.target.files[0]) self.decodeImage(e.target.files[0]); };
    ...
    this._keyHandler = e => {
      ...
      if (e.target && (e.target.tagName === "INPUT" || e.target.tagName === "TEXTAREA")) {
        // If focused in user/code input and enter pressed with pipe
  ...
  applyQr(data){
    ...
    // Strip trailing carriage returns/newlines from barcode scanners
    raw = raw.replace(/[\\r\\n]+$/, "").trim();
    let usr = "", pwd = "";
  ...
  async decodeImage(file){
    ...
    try {
      // 1. Try BarcodeDetector if supported natively
      ...
      reader.onload = () => {
        const img = new Image();
        img.onload = () => {
          const tryScales = [1.0, 800 / Math.max(img.width, img.height), 1200 / Math.max(img.width, img.height)];
          ...
          err.textContent = "No QR found in image. Please try another photo or paste badge code.";
        };
  ...
  openScanner(){
    ...
    if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia){
      err.textContent="Camera requires HTTPS or local connection. Upload QR image or paste code below.";
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment" } }).then(stream => {
      ...
      if (sBox) sBox.style.display = "none";
      err.textContent = "Camera access denied: " + e.message + ". Use 'Upload QR image' or paste code instead.";
      const f=document.getElementById("vpos-li-file");
      if(f) f.click();
    });
  },
...
print("Patched HTML length:", len(patched_html))

# Validate JS syntax
with open("c:/Users/josem/erpnext-system/vps_migration/temp_patched.html", "w", encoding="utf-8") as f:
    f.write(patched_html)

node_cmd = ["node", "-e",
    "const fs=require('fs');const h=fs.readFileSync('c:/Users/josem/erpnext-system/vps_migration/temp_patched.html','utf8');"
...
# Save to Web Page vehicle-pos-terminal
...
print("SUCCESSFULLY UPDATED Web Page vehicle-pos-terminal")
